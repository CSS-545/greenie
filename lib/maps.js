...
function withinDistance(coord_1, coord_2) {
  const ipLat = (parseFloat(coord_1.latitude) * Math.PI) / 180;
  const ipLog = (parseFloat(coord_1.longitude) * Math.PI) / 180;

  const gpsLat = (parseFloat(coord_2.latitude) * Math.PI) / 180;
  const gpsLog = (parseFloat(coord_2.longitude) * Math.PI) / 180;
  ...
  const distance = angluarDistance * 6371000;

  // console.log(distance);
  const response = distance < 250 ? true : false;
